...
path_to_TUs_sets_no_rRNA={'Exponential phase' : "F:\E_coli_RNA-Seq\E_coli_DY330_RNA-Seq\Expression_data\DY330_operons\DY330_RNA-Seq_operons_no_rRNA_EP_del_cor.txt",
                          ...
                          'Stationary phase' : "F:\E_coli_RNA-Seq\E_coli_DY330_RNA-Seq\Expression_data\DY330_operons\DY330_RNA-Seq_operons_no_rRNA_SP_del_cor.txt",
                          'Exponential phase old' : "F:\E_coli_RNA-Seq\E_coli_DY330_RNA-Seq\Expression_data\Incarnato_data\DOOR_Mu_del_cor_operons_expression_no_rRNA.txt"}
path_to_TUs_sets_no_rRNA_rpsl={'Exponential phase' : "F:\E_coli_RNA-Seq\E_coli_DY330_RNA-Seq\Expression_data\DY330_genes\DY330_RNA-Seq_genes_no_rRNA_rpsl_operon_EP_del_cor.txt",
                               'Early stationary phase' : "F:\E_coli_RNA-Seq\E_coli_DY330_RNA-Seq\Expression_data\DY330_genes\DY330_RNA-Seq_genes_no_rRNA_rpsl_operon_ESP_del_cor.txt",
                               'Stationary phase' : "F:\E_coli_RNA-Seq\E_coli_DY330_RNA-Seq\Expression_data\DY330_genes\DY330_RNA-Seq_genes_no_rRNA_rpsl_operon_SP_del_cor.txt",
                               'Exponential phase old' : "F:\E_coli_RNA-Seq\E_coli_DY330_RNA-Seq\Expression_data\Incarnato_data\DOOR_Mu_del_cor_genes_expression_no_rRNA_rpsl_operon.txt"}
path_to_TUs_sets_no_tRNA={'Exponential phase' : "F:\E_coli_RNA-Seq\E_coli_DY330_RNA-Seq\Expression_data\DY330_operons\DY330_RNA-Seq_operons_no_tRNA_EP_del_cor.txt",
                          'Early stationary phase' : "F:\E_coli_RNA-Seq\E_coli_DY330_RNA-Seq\Expression_data\DY330_operons\DY330_RNA-Seq_operons_no_tRNA_ESP_del_cor.txt",
                          'Stationary phase' : "F:\E_coli_RNA-Seq\E_coli_DY330_RNA-Seq\Expression_data\DY330_operons\DY330_RNA-Seq_operons_no_tRNA_SP_del_cor.txt",
                          'Exponential phase old' : "F:\E_coli_RNA-Seq\E_coli_DY330_RNA-Seq\Expression_data\Incarnato_data\DOOR_Mu_del_cor_operons_expression_no_tRNA.txt"}
...
def TU_association(GCSs_sets_dict, TU_set, set_type, window_width, path_out, set_name):
    ...
    fileout_US_GB_DS=open(path_out + set_name + '_numbers_of_associated_GCSs_US_GB_DS.txt', 'w')
    fileout_reduced=open(path_out + set_name + '_numbers_of_associated_GCSs_DS_only.txt', 'w')
    fileout_reduced_N3E=open(path_out + set_name + '_cumalative_N3E_of_associated_GCSs_DS_only.txt', 'w')
    if set_type=='16S operons':
        ...
        fileout.write("Operon_ID\tGenes_ID\tStart\tEnd\tStrand\tExpression\t") 
        fileout_US_GB_DS.write("Operon_ID\tGene_name\tStart\tEnd\tStrand\tExpression")  
        fileout_reduced.write("Operon_ID\tOperon_name\tStart\tEnd\tStrand\tExpression")  
        fileout_reduced_N3E.write("Operon_ID\tOperon_name\tStart\tEnd\tStrand\tExpression")  
        for GCSs_set_name, GCSs_set_data in GCSs_sets_dict.items():
            ...
            fileout_US_GB_DS.write("\tCondition\tGCSs in US\tGCSs in GB\tGCSs in DS")
            fileout_reduced.write(f'\t{GCSs_set_name} DSDS')
            fileout_reduced_N3E.write(f'\t{GCSs_set_name} DSDS')
        fileout.write("\n")
        fileout_US_GB_DS.write("\n")
        fileout_reduced.write("\n")
        fileout_reduced_N3E.write("\n")
    elif set_type=='genes':
        fileout.write("Gene_ID\tGene_name\tStart\tEnd\tStrand\tExpression")  
        fileout_US_GB_DS.write("Gene_ID\tGene_name\tStart\tEnd\tStrand\tExpression")  
        fileout_reduced.write("Gene_ID\tGene_name\tStart\tEnd\tStrand\tExpression")  
        fileout_reduced_N3E.write("Gene_ID\tGene_name\tStart\tEnd\tStrand\tExpression")  
        for GCSs_set_name, GCSs_set_data in GCSs_sets_dict.items():
            fileout.write("\tCondition\tGCSs in USUS\tGCSs in USGB\tGCSs in GBDS\tGCSs in DSDS")
            fileout_US_GB_DS.write("\tCondition\tGCSs in US\tGCSs in GB\tGCSs in DS")
            fileout_reduced.write(f'\t{GCSs_set_name} DSDS')
        ...
        fileout.write("Transcript_ID\tGene_name\tStart\tEnd\tStrand\tExpression")  
        fileout_reduced.write("Transcript_ID\tTranscript_name\tStart\tEnd\tStrand\tExpression") 
        fileout_reduced_N3E.write("Transcript_ID\tTranscript_name\tStart\tEnd\tStrand\tExpression") 
        for GCSs_set_name, GCSs_set_data in GCSs_sets_dict.items():
            fileout.write("\tCondition\tGCSs in USUS\tGCSs in USGB\tGCSs in GBDS\tGCSs in DSDS")
            fileout_reduced.write(f'\t{GCSs_set_name} DSDS')
            fileout_reduced_N3E.write(f'\t{GCSs_set_name} DSDS')
    ...
    for j in TU_set: #j - particular operon info (dictionary)  
        ...
        fileout_US_GB_DS.write(j['TUID'] + '\t' + j['TU name'] + '\t' + str(j['Start']) + '\t' + str(j['End']) + '\t' + j['Strand'] + '\t' + str(j['Transcription level']) + '\t')  
        fileout_reduced.write(j['TUID'] + '\t' + j['TU name'] + '\t' + str(j['Start']) + '\t' + str(j['End']) + '\t' + j['Strand'] + '\t' + str(j['Transcription level']) + '\t') 
        fileout_reduced_N3E.write(j['TUID'] + '\t' + j['TU name'] + '\t' + str(j['Start']) + '\t' + str(j['End']) + '\t' + j['Strand'] + '\t' + str(j['Transcription level']) + '\t') 
        ...
        for a, s in GCSs_sets_dict.items(): #a - Topo-Seq condition, s - corresponding set of GCSs.
            ...
            if set_type=='16S operons':
                ...
                fileout_US_GB_DS.write(str(stats[0]) + '\t' + str(stats[2]) + '\t' + str(stats[4]) + '\t') #US, GB, DS
                fileout_reduced.write(str(stats[4]) + '\t') #DSDS
                fileout_reduced_N3E.write(str(sum(N3E[4])) + '\t') #DSDS
                #USUS
                operons_stat_ds[a][0]+=stats[0]
                operons_stat_ds[a][1]+=N3E[0]
                operons_stat_ds[a][2]+=score[0] 
...
def write_GCSs_norm(GCSs_set_exp_interval_dict, path_out, set_name, set_type):
...
def TU_analysis_wrapper(GCSs_input_dict, score_inpath, TUs_sets_path, path_out, genome_len, window_width, set_type):
    #Reading input.
    ...
    for TU_set_name, TU_set_data in TU_sets_dict.items():
        ...
        GCSs_genes_assoc_info=TU_association(GCSs_sets_dict, TU_set_data, set_type, window_width, path_out, TU_set_name)
        TU_interval_stat_analysis(GCSs_sets_dict, GCSs_genes_assoc_info, TU_set_data, score_data, window_width, set_type, path_out, TU_set_name, genome_len)
        if set_type!="16S operons":
            ...
            GCSs_set_exp_interval_dict_g=GCSs_number_norm_16S(GCSs_genes_assoc_info, GCSs_sets_dict, genome_len)
        write_GCSs_norm(GCSs_set_exp_interval_dict_g, path_out, TU_set_name, set_type)
    return

set_type='operons'
#TU_analysis_wrapper(path_to_GCSs_files, Score_path, path_to_TUs_sets_full, TU_analysis_outpath_1, Genome_length, 1000, set_type)
#TU_analysis_wrapper(path_to_GCSs_files, Score_path, path_to_TUs_sets_full, TU_analysis_outpath_2, Genome_length, 5000, set_type)
#TU_analysis_wrapper(path_to_GCSs_files, Score_path, path_to_TUs_sets_no_rRNA_tRNA, TU_analysis_outpath_3, Genome_length, 1000, set_type)
TU_analysis_wrapper(path_to_GCSs_files, Score_path, path_to_TUs_sets_no_rRNA_tRNA, TU_analysis_outpath_4, Genome_length, 5000, set_type)
#TU_analysis_wrapper(path_to_GCSs_files, Score_path, path_to_TUs_sets_no_rRNA, TU_analysis_outpath_5, Genome_length, 1000, set_type)
#TU_analysis_wrapper(path_to_GCSs_files, Score_path, path_to_TUs_sets_no_rRNA, TU_analysis_outpath_6, Genome_length, 5000, set_type)
#TU_analysis_wrapper(path_to_GCSs_files, Score_path, path_to_TUs_sets_no_tRNA, TU_analysis_outpath_7, Genome_length, 1000, set_type)
#TU_analysis_wrapper(path_to_GCSs_files, Score_path, path_to_TUs_sets_no_tRNA, TU_analysis_outpath_8, Genome_length, 5000, set_type)
#TU_analysis_wrapper(path_to_GCSs_files, Score_path, path_to_TUs_sets_no_rRNA_rpsl, TU_analysis_outpath_9, Genome_length, 1000, set_type)
#TU_analysis_wrapper(path_to_GCSs_files, Score_path, path_to_TUs_sets_no_rRNA_rpsl, TU_analysis_outpath_10, Genome_length, 5000, set_type)
...
#Interval_analysis_wrapper(path_to_GCSs_files, Score_path, path_to_intervals_sets_4, Intervals_analysis_outpath, Genome_length)
